...
int checkEq(vector<double>& result, vector<double>& u, vector<double>& b, double aii, double aij, int N){
    ...
    #pragma omp parallel for collapse(2) shared(b,u) firstprivate(aij,aii,vec_size,innerLen) schedule(dynamic, innerLen)
    for (size_t i = 1; i < N; i++) // i is index of vector
        {
            for (size_t j = 1; j < N; j++)
            {
                int ind = i*N+j;
                result[ind] = aij*(u[ind+N]+u[ind+1]+u[ind-1]+u[ind-N]) + aii*u[ind] - b[ind];
            }
        }
    return 0;
}
...
int jacobiMethod(vector<double>& xk, const vector<double>& b, const double aii, const double aij, const int N, const double maxIter, const double maxTime_s){
    ...
    vector<double> xkp1(vec_size); // Vector initialized with 0
    ...
    for (size_t iteration = 0; iteration < maxIter; iteration++)
    {
        ...
        if(!stop){
            ...
            for (size_t i = 1; i < N-1; i++) // i is index of vector
            {
                for (size_t j = 1; j < N-1; j++)
                {
                    ...
                    //     temp = temp - xk[i+innerLen];
                    // }
                    xkp1[ind] = (b[ind]-aij*(xk[ind+N]+xk[ind+1]+xk[ind-1]+xk[ind-N]))*daii;
                }
            }
            #pragma omp single
            {
