...
int vecVecSub(vector<double>& c,vector<double>& a, vector<double>& b, size_t vec_size){
    assert(vec_size == c.size() and vec_size == a.size() and vec_size == b.size());
...
int checkEq(vector<double>& result, vector<double>& u, vector<double>& b, double aii, double aij, int N){
    ...
    for (size_t i = 0; i < vec_size; i++)
    {
        ...
        if (i>0 and i%innerLen!=0){
            tempDiff += aij*u[i-1];
        }
        if (i<(vec_size-1) and (i+1)%innerLen!=0){
            tempDiff += aij*u[i+1];
        }
...
int main(int argc, char *argv[]){
    ...
    double h, h2, dh2, x, y, eucNorm, maxNorm, aii, aij, itRuntime = 0.0;
    vector<double> u, up, A, b, result;
    ...
    for (size_t i = 0; i < vec_size; i++)
    {
        ...
        // Create vectors
        u.push_back(0); // Starting value fur u is 0
        up.push_back(up_func(x, y, k));
        result.push_back(0);
        if (vec_size-i<=(innerLen)) { // top most row has boundary condition
            b.push_back((f(x, y, k)+dh2*up_func(x,1,k))); 
        }
        else {
            b.push_back(f(x, y, k));
        }
